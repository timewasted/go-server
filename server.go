...
func (s *Server) initialTLSConfiguration() *tls.Config {
	return &tls.Config{
		...
		//   and speed, so try it first.
		// - ECDSA beats RSA on strength and speed, so try it first.
		// - AES-GCM is not vulnerable to any known attacks, so use it if at
		//   all possible.
		// - CBC-mode ciphers are a lesser evil compared to the other
		//   alternatives. It is vulnerable to BEAST (which is mitigated by
		...
		//   The alternative is RC4, which is pretty well broken these days.
		// - Fall back to 3DES as a last resort.
		CipherSuites: []uint16{
